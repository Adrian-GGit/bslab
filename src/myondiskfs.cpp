...
void MyOnDiskFS::buildStructure() {
    unsigned int numBlocks = sizeof(mySuperblock) % BLOCK_SIZE == 0 ? sizeof(mySuperblock) / BLOCK_SIZE : (sizeof(mySuperblock) / BLOCK_SIZE) + 1;
    unsigned int blockSize = numBlocks * BLOCK_SIZE;
